...
u32     videoTime[NUM_PERF_ITERATIONS + 1];
// RSP
u32     audioTime[NUM_PERF_ITERATIONS + 1];
...
void puppyprint_calculate_ram_usage(void) {
    ...
    ramsizeSegment[6] = (3000 * sizeof(struct SurfaceNode)) + (7000 * sizeof(struct Surface));
...
void puppyprint_render_profiler(void) {
    ...
    u32 i;
    s32 viewedNums;
    OSTime cpuCount = CYCLE_CONV(cpuTime+audioTime[NUM_PERF_ITERATIONS] + dmaAudioTime[NUM_PERF_ITERATIONS] + faultTime[NUM_PERF_ITERATIONS]+ videoTime[NUM_PERF_ITERATIONS]
